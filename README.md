...
```
**优点：**
子目录的cmake2写法简单，可以由各级目录自己控制，并且也会单独生成一个lib库，便于后期调试。
我的建议是，在分配任务的时候，每个人负责那的那片空间写一个cmake文件。

**本例特点：**
虽然上面的例子分了两个模块，但是本质上还是在同一个project中。
...
简单，易于理解，自己写自己用，非常方便
**缺点：**
有些库的头文件目录结构会比较复杂，如果这个库是给别人使用，需要配置的信息可能会比较复杂，并且如果还会修改的话（如：静态库变成动态库），两边对接就会不太方便。总之，这种方式不利于合作。
所以，理想的方式是配置文件交给库的编写者来实现，这样相对优雅
## 3.2 优雅方法 
...
### 3.2.3 优缺点
**优点：**
便于合作，负责app的人，不需要应为library的目录改动而修改cmake
**缺点：**
library的cmake将变的有些复杂。
# 四、如何优雅的兼容不同平台
> 名词解释：优雅==不需要大改
...
**结果：**
通过对mycomplie.toolchain.cmake的修改，可以看到输出结果不同

**如何看具体的配置信息：**
在执行`cmake .. `后，在`test4/library/build/CMakeFiles/mysum.dir/flags.make`中可以看到相关修改的配置信息。

## 4.3 优缺点
优点：
当需要对配置信息进行修改时，只需要改`mycomplie.toolchain.cmake`这个的信息即可。
